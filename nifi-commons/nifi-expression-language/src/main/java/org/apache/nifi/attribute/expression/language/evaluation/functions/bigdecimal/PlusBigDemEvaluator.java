 ...
 * BigDecimal 加法
 */
public class PlusBigDemEvaluator extends BigDecimalEvaluator {

    private final Evaluator<BigDecimal> subject;
    private final Evaluator<BigDecimal> plusValue;

    public PlusBigDemEvaluator(final Evaluator<BigDecimal> subject, final Evaluator<BigDecimal> plusValue) {
    ...
    public QueryResult<BigDecimal> evaluate(final Map<String, String> attributes) {
        ...
        final BigDecimal plus = plusValue.evaluate(attributes).getValue();
        if (plus == null) {
            return new BigDecimalQueryResult(null);
        }

         BigDecimal result = null ;
        if (subjectValue instanceof BigDecimal || plus instanceof BigDecimal) {
            result = (BigDecimal) subjectValue.add(plus);
        }
