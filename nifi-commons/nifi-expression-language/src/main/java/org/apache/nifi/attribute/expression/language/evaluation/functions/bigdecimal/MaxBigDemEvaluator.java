 ...
 * BigDecimal 最大值
 */
public class MaxBigDemEvaluator extends BigDecimalEvaluator {

    private final Evaluator<BigDecimal> subject;
    private final Evaluator<BigDecimal> plusValue;

    public MaxBigDemEvaluator(final Evaluator<BigDecimal> subject, final Evaluator<BigDecimal> plusValue) {
    ...
    public QueryResult<BigDecimal> evaluate(final Map<String, String> attributes) {
        ...
        final BigDecimal val = plusValue.evaluate(attributes).getValue();
        if (val == null) {
            return new BigDecimalQueryResult(null);
        }

        BigDecimal result  = (BigDecimal) subjectValue.max(val);
