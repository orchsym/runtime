...
/**
 * 
 * @author LiGuo BigDecimal 向左移n位
 */
public class MovePointLeftBigDemEvaluator extends BigDecimalEvaluator {

    private final Evaluator<BigDecimal> subject;
    private final Evaluator<String> n;

    public MovePointLeftBigDemEvaluator(final Evaluator<BigDecimal> subject, final Evaluator<String> n) {
    ...
    public QueryResult<BigDecimal> evaluate(final Map<String, String> attributes) {
        ...
        final Integer moveNum = Integer.parseInt(n.evaluate(attributes).getValue());

        BigDecimal result = (BigDecimal) subjectValue.movePointLeft(moveNum);
