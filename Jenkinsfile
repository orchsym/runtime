    ...
    stage('Compile') {
      steps {
        sh """
          ...
          if [[ "${BRANCH_NAME}" =~ '^PR.*' ]]; then mvn -T 4 package -Dorchsym.product.version=${env.VERSION_NAME}; else mvn -T 4 package -DskipTests -Dmaven.test.failure.ignore=true -Dorchsym.product.version=${env.VERSION_NAME}; fi
