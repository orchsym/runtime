...
public class ConvertJSONToXML extends AbstractProcessor {
    ...
    protected static final PropertyDescriptor JSON_PATH_EXPRESSION = new PropertyDescriptor.Builder()//
            ...
            .description("The json path expression to extract the contents")
            .required(false)
    ...
    protected static final PropertyDescriptor ELEMENT_NAME = new PropertyDescriptor.Builder()//
            ...
            .description("Specfic the element name. When user extracts content by Json Path expression, the content will be converted to xml format, and put into element with the element name. If the extracted content is JSONArray, then all element will put into a root element with name 'list'")
            .required(false)
            .addValidator(Validator.VALID)
            .build();
    ...
    protected static final PropertyDescriptor ENCODING = new PropertyDescriptor.Builder()//
            ...
            .description("Specfic the encoding name")
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .defaultValue("UTF-8")
            .build();
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        try {
            if (destinationContent) {
                ...
                FlowFile atFlowfile = session.putAttribute(flowFile, XML_ATTRIBUTE_NAME, xmlContent);
