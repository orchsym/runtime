 ...
 */
package org.apache.nifi.web.listentcpui.api;
...
import org.apache.nifi.web.listentcpui.model.UnPackingInfoCollectionEntity;

/**
 *
 */
@Path("/listentcp")
public class ListenTcpResource {

    private static final Logger logger = LoggerFactory.getLogger(ListenTcpResource.class);
    ...
    @Path("/info")
    public Response getListenTcpInfos(
        ...
        UnPackingInfoCollectionEntity unpackingInfos = getUnPackingInfos(configurationContext, requestContext);
        Gson gson = new Gson();
        String json = gson.toJson(unpackingInfos);
    ...
    @Path("/info")
    public Response creaListenTcpInfos(
            @Context final UriInfo uriInfo,
            final UnPackingInfoCollectionEntity infos) {
        ...
        saveListenTcpInfos(requestContext, infos);
    ...
    private UnPackingInfoCollectionEntity getUnPackingInfos(final NiFiWebConfigurationContext configurationContext, final NiFiWebRequestContext requestContext) {
        ...
        try {
            ...
            final String message = String.format("Unable to get UnPackingInfos[id=%s]: %s", requestContext.getId(), e);
            logger.error(message, e);
            throw new WebApplicationException(e, error(message));
        }

        UnPackingInfoCollectionEntity unpackingInfos = null;
        if (processorDetails != null) {
            try {
                String annotation = processorDetails.getAnnotationData();
                Gson gson = new Gson();
                unpackingInfos = gson.fromJson(annotation, new TypeToken<UnPackingInfoCollectionEntity>(){}.getType());
            } catch (final IllegalArgumentException iae) {
                final String message = String.format("Unable to deserialize existing UnPackingInfos for ListenTcp[id=%s]. Deserialization error: %s", requestContext.getId(), iae);
                logger.error(message, iae);
                throw new WebApplicationException(iae, error(message));
            }
        }
        // ensure the unpackingInfos isn't null
        if (unpackingInfos == null) {
            unpackingInfos = new UnPackingInfoCollectionEntity();
        }

        return unpackingInfos;
    }

        private void saveListenTcpInfos(final NiFiWebConfigurationRequestContext requestContext, UnPackingInfoCollectionEntity parameter) {
        ...
        try {
            ...
            final String message = String.format("Unable to save ListenTcpInfos[id=%s] in: %s", requestContext.getId(), e);
            logger.error(message, e);
        }
    }
